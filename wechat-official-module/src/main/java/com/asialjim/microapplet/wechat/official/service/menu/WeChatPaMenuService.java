 ...
 */
package com.asialjim.microapplet.wechat.official.service.menu;
...
import com.asialjim.microapplet.wechat.official.infrastructure.repository.menu.WeChatPaMenuRepository;
...
import com.asialjim.microapplet.wechat.official.remoting.menu.WeChatPublicAccountMenuRemoting;
...
import com.asialjim.microapplet.wechat.official.service.oauth.impl.MenuWeChatMpOAuthHandler;
...
public class WeChatPaMenuService {
    ...
    private final WeChatPublicAccountMenuRemoting weChatPublicAccountMenuRemoting;
    private final WeChatApplicationRepository.Aggregator applicationRepository;
    private final MenuWeChatMpOAuthHandler menuWeChatMpOAuthHandler;
    private final WeChatPaMenuRepository menuRepository;
    private final WeChatMpServerProperty weChatMpServerProperty;
     ...
     * @return {@link List<WeChatPaMenu> 公众号菜单列表}
     * @since 2023/2/8
     */
    public List<WeChatPaMenu> findWeChatMenus(String weChatId, String matchRule) {
        if (StringUtils.isBlank(matchRule)) matchRule = WeChatPaMenuRepository.DEFAULT_MATCH_RULE;
        return menuRepository.findWeChatPaMenus(weChatId, matchRule);
    }
     ...
     * @param paMenus   {@link List<WeChatPaMenu> 菜单列表}
     * @since 2023/2/8
     */
    public List<WeChatPaMenu> statementMenus(String weChatId, String matchRule, List<WeChatPaMenu> paMenus) {
        weChatId = this.applicationRepository.appByIndexThrowable(weChatId).getAppid();
        if (StringUtils.isBlank(matchRule))
            matchRule = WeChatPaMenuRepository.DEFAULT_MATCH_RULE;

        if (!StringUtils.equals(WeChatPaMenuRepository.DEFAULT_MATCH_RULE, matchRule)) {
            WeChatMenuMatchRule weChatPaMatchRule = menuRepository.findWeChatPaMatchRule(weChatId, matchRule);
     ...
     * @param matchRule {@link WeChatMenuMatchRule 匹配规则}
     * @return {@link List<WeChatMenuMatchRule> 该微信公众号匹配规则列表}
     * @since 2023/2/8
     */
    public List<WeChatMenuMatchRule> statementMatchRule(String weChatId, String name, WeChatMenuMatchRule matchRule) {
        menuRepository.addWeChatPaMatchRule(weChatId, name, matchRule);
        return menuRepository.findWeChatPaMatchRules(weChatId);
    }
     ...
     *
     * @param weChatId {@link String 公众号索引}
     * @return {@link List<WeChatMenuMatchRule> 匹配规则列表}
     * @since 2023/2/8
     */
    public List<WeChatMenuMatchRule> findWeChatMatchRules(String weChatId) {
        return menuRepository.findWeChatPaMatchRules(weChatId);
    }
    ...
    public void create(String weChatId, String matchRule) {
        ...
        menuWeChatMpOAuthHandler.deleteAuthPage(appid,authPageIds);

        List<WeChatPaMenu> menus = menuRepository.findWeChatPaMenus(appid, matchRule);

        WeChatMenuMatchRule menuMatchRule = menuRepository.findWeChatPaMatchRule(appid, matchRule);
        WeChatPublicAccountCreateMenuReq req = weChatMenu(appid, matchRule, menus);
        req.setMatchRule(menuMatchRule);

        WeChatApiRes weChatApiRes = StringUtils.equals(matchRule, WeChatPaMenuRepository.DEFAULT_MATCH_RULE)
    ...
    private WeChatPublicAccountCreateMenuReq weChatMenu(String appid, String matchRule, List<WeChatPaMenu> menus) {
        menus.sort(WeChatPaMenu::compareTo);

        val weChatMenu = new WeChatPublicAccountCreateMenuReq();
        List<WeChatPublicAccountMenuButton> buttons = new LinkedList<>();

        // 检查菜单配置
        final Map<Integer, List<WeChatPaMenu>> menuMap = menuConfigureCheck(menus);

        // 一级菜单配置个数不正确
        final int size = menuMap.size();
        if (size > 3)
            throw new IllegalArgumentException("一级菜单个数不能超过3个");

        for (int i = 1; i <= 3; i++) {
            final List<WeChatPaMenu> paMenus = menuMap.get(i);
            if (Objects.isNull(paMenus))
                continue;

            final WeChatPaMenu parentMenu = paMenus.stream().filter(item -> "parent".equals(item.getType())).findFirst().orElse(null);

            final WeChatPaMenu firstLevelMenu = Objects.isNull(parentMenu) ? paMenus.getFirst() : parentMenu;
            ...
            if (StringUtils.isBlank(button.getType()))
                paMenus.stream().filter(item -> !"parent".equals(item.getType())).sorted(WeChatPaMenu::compareTo).map(menu -> this.weChatMenuButton(appid, matchRule, menu)).forEach(button::addSubButton);
        ...
        // 非普通菜单
        if (!StringUtils.equals(matchRule, WeChatPaMenuRepository.DEFAULT_MATCH_RULE)) {
            WeChatMenuMatchRule weChatPaMatchRule = menuRepository.findWeChatPaMatchRule(appid, matchRule);
            if (Objects.isNull(weChatPaMatchRule))
    ...
    private WeChatPublicAccountMenuButton weChatMenuButton(String appid, String matchRule, WeChatPaMenu officialMenu) {
        WeChatPublicAccountMenuButton button = new WeChatPublicAccountMenuButton();
        final String handler = this.weChatMpServerProperty.getBaseUrl() + appid + "/oauth/page/menu";
        ...
        switch (type) {
            ...
            case "view":
                ...
                if (url.startsWith("https://open.weixin.qq.com")) {
                    ...
                    OAuthPageVo authPage = new OAuthPageVo();
                    authPage.setState(appid + "_menu_" + officialMenu.getFirst() + "_" + officialMenu.getSecond() + "_" + matchRule);
                    authPage.setUrl(url);
                    authPage.setAppid(appid);
                    authPage.setTimeout(-1L);
                    authPage = menuWeChatMpOAuthHandler.create(authPage);
                    String menuUrl = authPage.oauthUrl(handler);
                    button.setUrl(menuUrl);
                }
                break;
            case "miniprogram":
    ...
    private Map<Integer, List<WeChatPaMenu>> menuConfigureCheck(List<WeChatPaMenu> officialMenus) {
        ...
        // 按照一级菜单配置分类
        final Map<Integer, List<WeChatPaMenu>> menuMap = officialMenus.stream().collect(Collectors.groupingBy(WeChatPaMenu::getFirst));
        ...
        menuMap.forEach((key, value) -> {
            ...
            value.sort(WeChatPaMenu::compareTo);

            log.info("菜单： {}，  value:  {}", key, value);
            ...
            final Set<Integer> secondKeys = value.stream().map(WeChatPaMenu::getSecond).collect(Collectors.toSet());
            if (secondKeys.size() < subSize) {
                for (int i = 0; i < subSize; i++) {
                    WeChatPaMenu officialMenu = value.get(i);
