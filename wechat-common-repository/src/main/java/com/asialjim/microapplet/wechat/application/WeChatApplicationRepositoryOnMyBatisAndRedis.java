...
public class WeChatApplicationRepositoryOnMyBatisAndRedis {
    ...
    public WeChatApplicationRepository weChatApplicationRepositoryOnMyBatisAndRedis() {
        return new WeChatApplicationRepository() {

            @Override
            @Cacheable(cacheNames = WeChatApplicationRepository.WX_APP_CACHE, key = "'all-in-db'")
            public List<WeChatApplication> allApps() {
                return weChatApplicationMapperService.list();
            }
        };
    }
}